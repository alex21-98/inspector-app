    ...
    if not df_final.empty:
        ...
        if st.button("🚀 Generar Reporte Gerencial", type="primary", use_container_width=True):
            if not fundos_sel or not variedades_sel or not lotes_sel or not defectos_sel:
                ...
                colores_fuertes = ['#D32F2F', '#1976D2', '#388E3C', '#FBC02D', '#8E24AA', '#F57C00', '#0097A7', '#689F38', '#C2185B', '#111111']
                ...
                for defecto in defectos_sel:
                    for var in variedades_sel:
                        ...
                        if defecto in tolerancias_defectos:
                            ...
                            ax.text(x_pos_final, valor_tol_grafico, f' Tolerancia: {valor_tol}% ', color='white', fontsize=12, fontweight='bold',
                                    ha='right', va='bottom', zorder=15, bbox=dict(facecolor='#D32F2F', edgecolor='white', alpha=0.9, boxstyle='round,pad=0.3'))
